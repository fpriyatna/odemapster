...
public class TypeInferrer {
	...
	protected Map<Node, Set<AbstractConceptMapping>> mapInferredTypes = 
			new HashMap<Node, Set<AbstractConceptMapping>>();
	...
	public Map<Node, Set<AbstractConceptMapping>> infer(Op opQueryPattern) {
		this.mapInferredTypes = new HashMap<Node, Set<AbstractConceptMapping>>();
	...
	private void addToInferredTypes(Node node, AbstractConceptMapping cm) {
		Set<AbstractConceptMapping> types = this.mapInferredTypes.get(node);
		if(types == null) { 
	...
	private void inferByExpr(Expr expr) {
		if(expr.isFunction()) {
			...
			if((exprFunction instanceof E_Equals) || (exprFunction instanceof E_NotEquals) 
				...
				Set<AbstractConceptMapping> arg0Types = this.mapInferredTypes.get(arg0Node);
				...
				Set<AbstractConceptMapping> arg1Types = this.mapInferredTypes.get(arg1Node);
				...
				if(arg0Types != null && arg1Types == null) {
					...
					if(arg0Types.containsAll(arg1Types) && arg1Types.containsAll(arg0Types)) {
						...
						Set<AbstractConceptMapping> typesIntersection = 
