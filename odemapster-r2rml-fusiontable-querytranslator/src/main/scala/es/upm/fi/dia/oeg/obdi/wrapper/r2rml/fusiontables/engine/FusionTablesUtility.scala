...
class FusionTablesUtility {
  ...
  def showRows(tableId : String) {
    ...
    val sql = fusiontables.query().sql("SELECT Text,Number,Location,Date FROM " + tableId);
   ...
   * @throws IOException
   */
  def showRows2(tableId : String, columnName : String) {
    view.header("Showing Rows From Table");
   ...
   * @throws IOException
   */
  def describeTable(tableId : String) {
  ...
  def main(args: Array[String]): Unit = {
    ...
    fusionTablesUtility.listTables();
//    
