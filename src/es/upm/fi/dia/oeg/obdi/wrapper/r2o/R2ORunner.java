...
public class R2ORunner extends AbstractRunner {
	...
	public void run(String r2oConfigurationFile) throws Exception {
		...
		try {
			...
			R2OMappingDocument r2oMapping = (R2OMappingDocument) parser.parse(r2oAbsolutePath);


			//test the parsing result
			parser.testParseResult(r2oAbsolutePath, r2oMapping);
			...
			Collection<AbstractConceptMapping> conceptMappings = r2oMapping.getConceptMappings();
			for(AbstractConceptMapping conceptMapping : conceptMappings) {
				//unfold mapped concepts
				AbstractUnfolder unfolder = new R2OUnfolder();
				String sqlQuery = unfolder.toSQL(conceptMapping);
				...
				if(splitOutput) { //if split output is true, then always create a new model and write it to the output
					...
					if(model == null) {
						model = modelGenerator.createModel(jenaMode, r2oMapping.hashCode() + "");
					}
